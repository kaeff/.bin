...
def input_to_regex(date_input):
    return re.compile(date_input.replace('YYYY', r'\d{4}').replace('YY', r'\d{2}').replace('MM', r'\d{2}').replace('DD', r'\d{2}'))
...
def convert_date_format(filename, old_format, new_format):
    ...
    if match:
        ...
        try:
            date_obj = datetime.strptime(date_str, input_to_python(old_format))
            if date_str != datetime.strftime(date_obj, input_to_python(old_format)):
                raise ValueError('Date format does not match input format.')
        except ValueError:
            print(f"Error: {old_regex} does not match the format {input_to_python(old_format)}. Skipping file {filename}.")
            # print(f"Error: {old_regex};{date_str};{input_to_python(old_format)};{date_obj};{filename}.")
            return filename

        new_date_str = datetime.strftime(date_obj, input_to_python(new_format))
        new_filename = filename.replace(date_str, new_date_str, 1)
        return new_filename
...
def apply_renamings(directory_path, renamings):
    ...
    Args:
        ...
        renamings (dict): A dictionary of renamings to be made, mapping old filenames to new filenames.
...
def rename_files_in_directory(directory_path, old_format, new_format, ask_for_consent = True):
    """
    Renames all files in a directory whose filename starts with the expected date pattern.

    Args:
        directory_path (str): The path to the directory containing the files to be renamed.
        old_format (str): The current date format in the filename.
        new_format (str): The desired date format in the filename.
        ask_for_consent (bool): If False, skips user input prompt and renames files directly

    Returns:
        None
    """
    print("Renaming matching files in directory: " + directory_path)
    ...
    if ask_for_consent:
        confirmation = input("Do you want to proceed with the renaming? (Y/N) ").strip().lower()
        if confirmation != 'y':
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Rename files with date format in filename.')
